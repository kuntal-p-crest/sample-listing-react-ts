...
app.get("/users/:id/relationships/logins", (req, res) => {
    try {
        ...
        user_logins[id - 1].logins.map((details)=>{
            var geo = geoip.lookup(details.ip_v4);
            if (geo)
        ...
        console.log("error in relation", err);
    }
});

app.get("/users/country/:ip", (req, res) => {
    const { ip } = req.params;
    var geo = geoip.lookup(ip);
    if (geo)
        return res.send(geo.country);
    return null;
});
